...
# Path of CSV file
LOG_PATH = 'D:/Work/Timeseries_models/DATA/DPF_data_55T/Log_Data_Blazo55T.csv'
local_filename = "D:/Work/Timeseries_models/DATA/DPF_data_55T/test.json"
Temp_tar_file = 'D:/Work/Timeseries_models/DATA/DPF_data_55T/test.tar.gz'
...
Data_cnt = 33
for veichle_cnt in range(15,len(veichle)):
#for veichle_cnt in range(9,10):
    print('----------------------------------------------------------',veichle_cnt)
    ...
    ID_stamp_col = veichle_select["id"]
    veichle_ID = ID_stamp_col[0][0:18]
    print(veichle_ID)
    #exit(0)
    for state_cnt in range(0,veichle_select.shape[0]):
        if ((Transition_col[state_cnt] == 'active-removed') and ((state_cnt+1) < veichle_select.shape[0])):
            ...
            if((Engine_run_col[state_cnt] < 1) or (Engine_run_col[state_cnt+1] < 1)):
                ...
                # FAULT STATE
                URL = "https://apis.intangles.com/vehicle/" + veichle_ID +"/obd_data/"+ str(int(A_state_start)) + "/" + str(int(A_state_end)) +"?fetch_result_from_multiple_sources=true"
                print(URL)
                print('----------------------------------------------------------',veichle_cnt)
                DownloadFile(URL,local_filename)
                out_path = 'D:/Work/Timeseries_models/DATA/DPF_data_55T/A_State/' + "State" + str(Data_cnt)
                ExtractJason(local_filename,Temp_tar_file,out_path)
                # NORMAL STATE
                URL = "https://apis.intangles.com/vehicle/" + veichle_ID +"/obd_data/"+ str(int(N_state_start)) + "/" + str(int(N_state_end)) +"?fetch_result_from_multiple_sources=true"
                print(URL)
                print('----------------------------------------------------------',veichle_cnt)
                DownloadFile(URL,local_filename)
                out_path = 'D:/Work/Timeseries_models/DATA/DPF_data_55T/N_State/' + "State" + str(Data_cnt)
                ExtractJason(local_filename,Temp_tar_file,out_path)
                Data_cnt = Data_cnt + 1
